(function() {
  ...
  $(function() {
    return $('#auto_select').click(function() {
      return getLocation(function(position) {
        return reverseGeoCode(position.coords.latitude, position.coords.longitude, position.coords.accuracy, function(result) {
          if (allocationManager === null) {
            allocationManager = new AllocationManager();
          }
          return allocationManager.getAllocationData(result['Feature'][0]['Property']['Address'], function(chiku_list) {
            var chiku, key_value_list;
            console.log(JSON.stringify(chiku_list));
            key_value_list = generateKeyValueList(chiku_list);
            if (key_value_list.length === 1) {
              chiku = key_value_list[0]['chiku'].replace("町", "");
